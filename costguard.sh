...
while true; do
  ...
  for entry in ${=PODS}; do
    ...
    eval "$(cat $sshf)"
    info=$(ssh -i "$KEY" -o StrictHostKeyChecking=no -o ConnectTimeout=20 -p "$PORT" root@"$IP" 'u=$(nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits | head -1); n=$(ps -eo args | grep -E "python (train|baselines|bench)\.py" | grep -v grep | wc -l); w=$(ps -eo args | grep -E "pod[0-9a-z_]*(_boot|_go|_runs|_post|_gate|_jev)|ladder|mix|typed|eval_wf|jevbench_run|bench\.py|scripts/compose|hf upload|hf download|uv sync|pip install" | grep -v grep | wc -l); echo "$u $n $w"' 2>/dev/null)
