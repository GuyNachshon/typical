...
- **Typed primitives** (PLAN7 Track C, new in this release):
  ...
  - **Score** — intended as K-way Choice with ordinal-smoothed targets (τ = 0.7, §3ac's adopted
    recipe). **Caveat:** `runs/ts1b/results.json["args"]` records `score_head: "choice"` and
    `ordinal_smooth: 0.0` for this checkpoint — the smoothing flag is not actually set, despite
    §3ae's run title crediting it. Flagging this discrepancy rather than silently correcting it;
    not yet root-caused. Score should currently be treated as plain K-way Choice in `ts1b`, not as
    the ordinal-smoothed variant validated in §3ac.

## Training args (verbatim from `results.json["args"]`, run `ts1b`)
...
- The Score-head ordinal-smoothing discrepancy above (`ordinal_smooth: 0.0` logged despite §3ac's
  adopted recipe) — treat Score as plain K-way Choice, not ordinal-smoothed, until re-verified.
