...
- **Typed primitives** (PLAN7 Track C, new in this release):
  ...
  - **Score** — K-way Choice trained with ordinal-smoothed targets (τ = 0.7, §3ac's adopted recipe).
    Note: `results.json["args"]` was regenerated by an `--eval_only` rerun (to restore missing MMLU
    sets) and therefore records the *eval* command's flags (`ordinal_smooth: 0.0`); the training
    args stored inside `best.pt` are authoritative: `ordinal_smooth 0.7`, `noul_head bern`,
    `family_weights E:0.40,K:0.15,W:0.35,U:0.10`, `null_aug W:0.20`, `max_state 1024`, `bs 64`,
    `grad_accum 4`, `steps 12000`. The original results file is preserved as `results_nomm.json`.

## Training args (from `results.json["args"]` of the eval-only rerun — see the Score note above; `best.pt` holds the training args)
