...
(\Cref{sec:train:calibration}), and cost (\Cref{sec:train:latency}). Two runs remained in flight at
the time of writing (\texttt{ts1b\_semif}, \texttt{tl1b\_semif}) and are omitted rather than reported
provisionally; a Qwen3.5-9B checkpoint (\texttt{tl2}) has hard-tier results only and appears solely in
the resolution argument below, not in \Cref{tab:headline}.
...
\textbf{The hard tier cannot resolve neighbouring rungs at all.} A Qwen3.5-9B checkpoint trained with
the same recipe scores exactly 55/111 on the hard tier, the same count as the Qwen3.5-4B checkpoint in
\Cref{tab:headline}. This is not a tie between equal models: the two disagree on 26 individual items
(13 in each direction) and not one of their 111 probability vectors matches. McNemar's exact test gives
$p = 1.000$ with a paired 95\% interval on the difference of $[-.090, +.090]$. The correct reading is
that \emph{this benchmark is uninformative about 4B-vs-9B at this sample size}, not that the two models
perform equally, and the same caution applies to any adjacent pair of rungs in \Cref{tab:headline}.

The hard tier also moves in the opposite direction from the standard tier, which is the subject of
\Cref{sec:boundary}.
