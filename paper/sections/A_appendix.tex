...
\paragraph{Evaluation sets.} Banking77 is scored against all 77 labels. The MMLU-Pro probe is scored among
...
\texttt{policy\_permit} ($\numcands{=}2$, floor .612) and \texttt{action\_select} ($\numcands{=}4$, floor
.233). The evaluation-only external sets are PagerDuty (alert triage, yes/no, majority floor .792),
...
\paragraph{JevBench.} Each item carries a state, a rubric and an exact label set. The hard tier spans
...
is why every number here is on the 231 public ids and unranked. The paraphrase pairs share a
\texttt{group} field, over which the cluster bootstrap resamples.
...
($+.023$, $-.026$), the spread 1{,}200 items allow.
...
to .28 relative to 6A, and gave the best held-out Score NLL of the sweep (1.26). Null-augmentation
