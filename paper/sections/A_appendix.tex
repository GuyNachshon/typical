...
\begin{figure}[h]
  ...
  \caption{\textbf{Public subset, unranked.} Per-family JevBench-hard accuracy for the two released
  models (\texttt{ts1b}, \texttt{tm1b}), the unreleased \texttt{tl1b}, and the frozen-14B 3-shot
  control, chance line at .336. Per-family $n$ ranges from 4 to 19 items and no single family
  separates; the uniform direction across the serial families is the observation
  (\Cref{sec:boundary:hard}). Per-family values are re-derived directly from each run's JSON artefact
  rather than transcribed from prose, and differ slightly from \Cref{sec:boundary:frozen}'s aggregate
  table in a small number of cells as a result; the JSON is the source of truth.}
...
\begin{figure}[h]
  ...
  p50 latency (ms, log scale) for the two released models, with generative-LLM decode-latency
  comparison bands. Latency is the served path with the state prefix already cached
  (\texttt{runs/serve\_bench2}, $\numcands{=}2$, 256-token state, one stream, in process, model load
  excluded), i.e.\ the same quantity as \Cref{app:serving}, not the single-decision ladder of
  \Cref{tab:app-latency}, which includes the state encode on the unoptimized path. Referenced from
...
ablation, and the purpose-built held-out long-state set -- which changed several claims in earlier
drafts of this paper in both directions: it weakened the frozen-teacher distillation and standard-tier
bundle results to the form they now take in \Cref{sec:boundary:frozen}, and it turned the long-state
truncation diagnosis from an unsupported attribution into the measured $2\times2$ of
...
text; the items, labels, and candidate sets are identical. It shares a generator family with training
data, which is why \Cref{sec:boundary:truncation} scopes its conclusion to the training defect rather
than to long-context capability in general. Significance for the matched-vs-matched comparison is a
two-proportion test over the 605 paired items ($z = 6.2$, $p = 5\times10^{-10}$ at 1.7B;
$p = 7\times10^{-12}$ at 14B).
