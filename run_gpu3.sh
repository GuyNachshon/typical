...
schedule() {
  ...
  run_if_needed main_v2        uv run --no-sync python train.py --name main_v2 --tap_layer 20 --zscore $LORA_FLAGS --wandb --hf_repo "$HF_REPO"
  run_if_needed main_v3        uv run --no-sync python train.py --name main_v3 --tap_layer 20 --zscore $LORA_FLAGS --data data_v3 --steps 24000 --wandb --hf_repo "$HF_REPO"
  run_if_needed abl_nohybrid   uv run --no-sync python train.py --name abl_nohybrid --tap_layer 20 --zscore $LORA_FLAGS --data data_v3 --no_hybrid --wandb --hf_repo "$HF_REPO"
  run_if_needed main_v3_s1     uv run --no-sync python train.py --name main_v3_s1 --tap_layer 20 --zscore $LORA_FLAGS --data data_v3 --seed 1 --wandb --hf_repo "$HF_REPO"
  run_if_needed bench          uv run --no-sync python bench.py --model runs/main_v3 --backbone Qwen/Qwen3-1.7B-Base --name bench
