...
# Typical decides.
Give it text, a question, and the answers you will accept. It scores each answer and flags when none of them fit.
...
- 45 ms a decision
- 80.4% right on 150 intents
- 83.6% on rules it never saw
- Open weights

**01 — What it is**

## State in. A typed decision out.
Typical is a decision model. Your code hands it text it already has, a question, and the answers it is willing to accept. Typical returns how likely each answer is, plus the odds that none of them apply. It never writes a sentence, so there is nothing to parse and no answer can come back that you did not list.
One call, 45 ms, the same shape every time. Cheap enough to run on every step of a workflow, not only the steps you can afford a chat model for.

**The same job, as a decision**

**02 — Results**

## 80.4% right on 150 intents. 45 ms a decision. Both with a control beside them.
Every number below sits next to the same model before we trained it, so you can see what the training bought and what it cost. On the hard tier it bought nothing: both sizes sit at chance there, and say so too confidently.
...
### Yes / no
A Bernoulli head on the same cached state. The only query type whose answer is exactly order-invariant.

### Score
An ordered level, 0 through N, with the expected level under the candidates.
It is not an LLM replacement or a knowledge model. It decides from the text you give it (≤ 1k tokens); it does not plan, compute, or reason over long documents, and on hard multi-step workflow judgments it is still at chance.
...
**The model picks each manoeuvre · 123 of 127 matched the rule list**
Each tick the road is rendered as the situations that apply, in a fixed order; seven one-condition rules are in the question; the model picks one manoeuvre from the legal set. Over 862 m of the recorded run it stopped at all eight red lights, overtook three times, and disagreed with the rule list four times — once badly enough to clip the car ahead. Click the road to drop a cone, a pedestrian or a stopped car in its path and watch the probabilities move. It still cannot take a rule with an “or” in it.

**Snake · canvas**
One sentence per tick; the candidate set is the safe moves. 98.4% of moves on 62 boards (chance 48%); 24 food in the recorded run. _(fine print)_

**DOOM · E1M1 · live**
REC · typical-small · E1M1
booting
Chocolate Doom, shareware 1.9, the frame exactly as the engine draws it. Two facts per tick (badly hurt? an enemy in sight?), the model picks retreat, engage or explore, and aims from “the player must turn left to face the imp”. Over a recorded 200-decision run on E1M1 it matched the rule list every time, fired 12 shots and finished on 11 health — most of its calls near the end were retreat (site/data/replays/realdoom.json). The kill counter that run reported does not reconcile with the shots fired, so it is not quoted here. _(fine print)_
...
**05 — What does not work**

## Five things it gets wrong, and why.
- Object-location sentences. Doom’s “the enemy is 5 cells to the left” never fired a turn (0 of 12); the model aims only once the state says “the player must turn left to face the enemy”. Numbers it must compare itself (“41 health”) leak into the decision: with them, an enemy in the crosshair at mid health read as retreat. Snake and Doom both need the engine to pre-compute the comparison.
- Rules with "or" in them, negated conditions, or more than four rules before the default. Drive holds its score only with the state sentences in a fixed order; shuffled, accuracy drops into the high eighties.
- Compaction: P(keep) never crosses .5, so there is no usable keep/drop threshold, only a ranking.
- Level-7 composition (temporal, units, expected value, trade-off): at chance on every checkpoint.
- Long documents: states above ~1k tokens. The cause is a truncation bug in the training data; a fix is in training.

**06 — Get started**

## Three commands and you are running it locally.
The public inference package ships inside the model repos on Hugging Face. Download the inference/ folder, install its requirements, and call it. state is a string or a dict; candidates are whatever you pass, per call, with no retraining.
The three calls themselves are the six lines shown above under _(fine print)_
: choice, noul, score, each taking the same state. _(fine print)_
`[link] Read the research: architecture, data, 130 runs.Research →`
`[link] Get the weights: 1.7B and 4B, open.Hugging Face →`
Models that decide, not generate.

**Weights and inference code on Hugging Face. Training code to follow.**
...
**Results table** — one row per released model:

- typical-small-preview: JevBench std 0.750, hard 0.387, CLINC-150 0.734, 45 ms at K=2
- typical-small: JevBench std 0.694, hard 0.432, CLINC-150 0.804, 45 ms at K=2
- typical-medium: JevBench std 0.806, hard 0.423, CLINC-150 0.847, 56 ms at K=2

**In training box** — a 14B release candidate, not yet a release (Qwen3-14B, facts-first long rows, 3,072-token states, frozen-14B distillation) and tm2 (Qwen3.5-4B); pass rule: hard >= .559 or hard Brier <= .65, long_policy >= .35 (REPORT §3ag)
