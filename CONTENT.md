...
No generation. No parsing. _(fine print)_
...
- Same facts, different rule, different decision
- 16 ms a decision, warm
...
### Noul · yes/no
P(yes) for a proposition, off the same cached state. The only question type whose answer is exactly order-invariant.
...
booting
Chocolate Doom, shareware 1.9, the frame exactly as the engine draws it. Two facts per tick (badly hurt? an enemy in sight?), the model picks retreat, engage or explore, and aims from “the player must turn left to face the imp”. The pipeline is engine → text state → Typical → action: it never receives a pixel. Over a recorded 200-decision run on E1M1 it matched the rule list every time, fired 12 shots and finished on 11 health — most of its calls near the end were retreat (site/data/replays/realdoom.json). The kill counter that run reported does not reconcile with the shots fired, so it is not quoted here. _(fine print)_
...
### Good fit
- Routing and triage
- Classification over labels you define
- Policy and eligibility checks
- Extraction with a closed answer set
- Scoring on a scale you define
- Tool choice and next action

### Not yet
- Documents past the state window
- Serial arithmetic
- Temporal composition
- Multi-step planning
- Vision, or any raw perception
