...
schedule() {
  run_if_needed diag_tap20_frozen uv run python train.py --name diag_tap20_frozen --tap_layer 20 --lora_r 0 --wandb --hf_repo "$HF_REPO"
  run_if_needed diag_tap20        uv run python train.py --name diag_tap20 --tap_layer 20 --wandb --hf_repo "$HF_REPO"
